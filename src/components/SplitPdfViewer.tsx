...
export function SplitPdfViewer() {
  ...
  const [textItemsByPane, setTextItemsByPane] = useState<TextItemsByPane>({ left: [], right: [] });
  ...
  const canUseFullscreen = (hasLeftPdf || hasRightPdf) && !isSplitMode;

  const getEffectivePane = useCallback((): PaneId | null => {
    if (panes[activePane]?.pdfUrl) return activePane;
    if (panes.left.pdfUrl) return "left";
    if (panes.right.pdfUrl) return "right";
    return null;
  }, [activePane, panes]);
  ...
  const moveActivePanePage = useCallback((delta: number) => {
    ...
    const nextPage = current.totalPages > 0 ? Math.min(Math.max(1, proposedPage), current.totalPages) : Math.max(1, proposedPage);
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      if (event.deltaY < 0) zoomIn(paneId); else zoomOut(paneId);
      window.setTimeout(() => { wheelLockRef.current = false; }, 40);
    };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (isCtrlOrMeta && event.key.toLowerCase() === "f") {
        ...
        window.setTimeout(() => window.dispatchEvent(new CustomEvent("pdf-viewer-open-search")), 0);
        return;
      }
  ...
  const handleTextItems = useCallback((pane: PaneId, page: number, items: PdfTextItem[]) => {
    setTextItemsByPane((prev) => {
      ...
      return { ...prev, [pane]: [...filtered, ...items] };
    });
  }, []);
  ...
  const formulas = useMemo<FormulaCandidate[]>(() => {
    ...
    if (hasLeftPdf) result.push(...extractFormulaCandidates("left", panes.left.pageNumber, textItemsByPane.left));
    if (hasRightPdf) result.push(...extractFormulaCandidates("right", panes.right.pageNumber, textItemsByPane.right));
    return result;
  ...
  const renderedPaneNodes = visiblePanes.flatMap((pane, index) => {
    ...
    if (isSplitMode && index > 0) {
      nodes.push(<div key="split-divider" className="split-divider" onPointerDown={handleDividerPointerDown} onDoubleClick={resetDivider} title="ドラッグで左右幅調整 / ダブルクリックで50:50" />);
    }
    nodes.push(<PdfPane key={pane} pane={pane} debugTextLayer={debugTextLayer} isActive={isFullscreen ? activePane === pane : false} fullscreenMode={isFullscreen} onTextItems={handleTextItems} />);
    return nodes;
  });
  ...
  return (
    <div ref={appRootRef} className={["app-layout", isSidebarCollapsed ? "sidebar-collapsed" : "sidebar-expanded"].filter(Boolean).join(" ")}>
      <Sidebar
        ...
        onToggleSidebar={() => { clearOutlineHighlight(); setIsSidebarCollapsed(true); }}
        onToggleDebugTextLayer={() => { clearHighlight(); setDebugTextLayer((value) => !value); }}
        onToggleFullscreen={toggleFullscreen}
      />

      <main
        ref={viewerAreaRef}
        className={isSplitMode ? (isFullscreen ? "viewer-area split-mode fullscreen-mode" : "viewer-area split-mode") : (isFullscreen ? "viewer-area single-mode fullscreen-mode" : "viewer-area single-mode")}
        style={isSplitMode ? { gridTemplateColumns: `minmax(0, ${splitRatio}fr) 8px minmax(0, ${100 - splitRatio}fr)` } : undefined}
      >
        {visiblePanes.length === 0 && <div className="no-pdf-message">PDFを読み込んでください</div>}
        {renderedPaneNodes}
      </main>
    </div>
  );
